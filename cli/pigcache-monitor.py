...
------------
  * Python 3.6+
  * redis-py            (pip3 install --user redis)
  * mysql-connector-python OR PyMySQL (only for `mysql` / `snapshot` MySQL section)
"""
...
def connect_redis(args, cfg):
    """Return a connected redis.Redis client, or raise SystemExit."""
    if redis_lib is None:
        print("error: the 'redis' package is missing. "
              "Run: pip3 install --user redis", file=sys.stderr)
        sys.exit(2)
    ...
    try:
        ...
        print(f"error: cannot reach Redis at {host}:{port} (db {db}): {exc}",
              file=sys.stderr)
        sys.exit(2)

    return client, {"host": host, "port": port, "db": db}
...
def render_snapshot_text(snap, cb_state):
    ...
    out.append(f"  endpoint                : {e['host']}:{e['port']} (db {e['db']})")
