...
// ── PRO_END ───────────────────────────────────────────────────────────────────
	...
	public static function handle_post() {
		...
		if ( isset( $_POST['pigcache_save_ttl'] ) ) {
			...
			$sql  = isset( $_POST['pigcache_sql_ttl'] ) ? absint( $_POST['pigcache_sql_ttl'] ) : PigCache_Config::DEFAULT_SQL_TTL;
			$html = isset( $_POST['pigcache_html_ttl'] ) ? absint( $_POST['pigcache_html_ttl'] ) : PigCache_Config::DEFAULT_HTML_TTL;
...
// ── PRO_START ─────────────────────────────────────────────────────────────────
		self::render_query_analytics_card();
// ── PRO_END ───────────────────────────────────────────────────────────────────
...
// ── PRO_START ─────────────────────────────────────────────────────────────────
		...
		self::maybe_render_continuous_learning();
		self::render_adaptive_ttl_section();
		self::render_license_section_pro();
// ── PRO_END ───────────────────────────────────────────────────────────────────
	...
	private static function render_flash_notices() {
		if ( isset( $_GET['pigcache_err'] ) ) {
			echo '<div class="notice notice-error"><p>' . esc_html( sanitize_text_field( wp_unslash( rawurldecode( (string) $_GET['pigcache_err'] ) ) ) ) . '</p></div>';
		}
		...
		if ( isset( $_GET['pigcache_dropins_removed'] ) && is_string( $_GET['pigcache_dropins_removed'] ) ) {
			...
			$parts   = array_filter( array_map( 'trim', explode( ',', sanitize_text_field( wp_unslash( $_GET['pigcache_dropins_removed'] ) ) ) ) );
			$labels  = array();
			foreach ( $parts as $p ) {
				if ( isset( $allowed[ $p ] ) ) {
					$labels[] = $p;
...
// ── PRO_START ─────────────────────────────────────────────────────────────────
	...
	public static function handle_profiler_post() {
		...
		if ( 'start_learning' === $action ) {
			$days = isset( $_POST['pigcache_learn_days'] ) ? absint( $_POST['pigcache_learn_days'] ) : 7;
	...
	public static function handle_learning_post() {
		...
		if ( 'enable' === $action || 'disable' === $action ) {
			...
			if ( isset( $_POST['pigcache_sample_rate'] ) ) {
				$rate                   = (float) sanitize_text_field( wp_unslash( $_POST['pigcache_sample_rate'] ) ) / 100;
	...
	public static function handle_adaptive_ttl_post() {
		...
		if ( 'save' === $action ) {
			...
			if ( isset( $_POST['pigcache_ttl_hot_stable'] ) ) {
				update_option( 'pigcache_ttl_hot_stable', max( 1, absint( $_POST['pigcache_ttl_hot_stable'] ) ), false );
			}
			if ( isset( $_POST['pigcache_ttl_hot_dynamic'] ) ) {
				update_option( 'pigcache_ttl_hot_dynamic', max( 1, absint( $_POST['pigcache_ttl_hot_dynamic'] ) ), false );
			}
			if ( isset( $_POST['pigcache_traffic_cold_threshold'] ) ) {
				update_option( 'pigcache_traffic_cold_threshold', max( 0, absint( $_POST['pigcache_traffic_cold_threshold'] ) ), false );
			}
			if ( isset( $_POST['pigcache_mutation_dynamic_threshold'] ) ) {
				update_option( 'pigcache_mutation_dynamic_threshold', max( 0, absint( $_POST['pigcache_mutation_dynamic_threshold'] ) ), false );
			}
